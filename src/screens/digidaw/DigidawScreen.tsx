...
import { digidawService, type Category } from '../../services/digidawService';
...
const DigidawScreen: FC = () => {
	...
	React.useEffect(() => {
		const fetchCategories = async () => {
			try {
				const data = await digidawService.getCategories();
				setCategories(data);
			} catch (e) {
				console.error(e);
			}
					...
					<View style={[styles.cardGrid, { columnGap: cardGap, rowGap: cardGap, gap: cardGap }]}>
							...
							<Pressable
								key={category.id}
											...
											accessibilityLabel={`Buka kategori ${category.name}`}
							>
								<View
									style={[
										styles.iconWrapper,
