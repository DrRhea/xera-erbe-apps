...
const moduleBadge = require('../../../assets/images/poke.png');

const clamp = (value: number, min: number, max: number) => Math.min(Math.max(value, min), max);

type PokeQuestionRoute = RouteProp<RootStackParamList, 'PokeQuestion'>;
...
const PokeQuestionScreen: FC = () => {
  const route = useRoute<PokeQuestionRoute>();
  ...
  // Poke specific palette (can be customized)
  ...
  const handleSelectOption = useCallback(
    async (optionId: string) => {
      ...
      try {
        await pokeService.recordAnswer(attemptId, currentQuestion.id, optionId);
      } catch (error) {
...
export default PokeQuestionScreen;
