...
import { pokeService, type PokeModule } from '../../services/pokeService';

const pokeIcon = require('../../../assets/images/poke.png');
...
const PokeScreen: FC = () => {
  ...
  const [modules, setModules] = useState<PokeModule[]>([]);
  const [isLoading, setIsLoading] = useState(false);

  useEffect(() => {
    const fetchModules = async () => {
      try {
        const data = await pokeService.getAllModules();
        setModules(data);
      } catch (e) {
        console.error(e);
      }
  ...
  const handleModulePress = useCallback(async (module: PokeModule) => {
    if (isLoading) return;
    setIsLoading(true);
    try {
      const session = await pokeService.startSession(module.id);
      navigation.navigate('PokeQuestion', {
        moduleId: module.id,
        ...
        <View style={[
          ...
          <View style={{ gap: cardGap }}>
            {modules.map((module) => (
              ...
              >
                <Image source={pokeIcon} style={styles.cardIcon} resizeMode="contain" />
