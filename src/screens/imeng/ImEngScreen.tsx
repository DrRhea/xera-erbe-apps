...
import { imengService, type ImengModule } from '../../services/imengService';

const imengIcon = require('../../../assets/images/imeng.png');
...
const ImengScreen: FC = () => {
  const navigation = useNavigation<NavigationProp<RootStackParamList>>();
  const layout = useResponsiveLayout();
  const [modules, setModules] = useState<ImengModule[]>([]);
  const [isLoading, setIsLoading] = useState(false);

  useEffect(() => {
    const fetchModules = async () => {
      try {
        const data = await imengService.getAllModules();
        setModules(data);
      } catch (e) {
        console.error(e);
      }
  ...
  const handleModulePress = useCallback(async (module: ImengModule) => {
    if (isLoading) return;
    setIsLoading(true);
    try {
      const session = await imengService.startSession(module.id);
      navigation.navigate('ImEngQuestion', {
        moduleId: module.id,
        ...
        <View style={[
          ...
          <View style={{ gap: cardGap }}>
            {modules.map((module) => (
              ...
              >
                <Image source={imengIcon} style={styles.cardIcon} resizeMode="contain" />
