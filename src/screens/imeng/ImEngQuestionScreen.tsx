...
const moduleBadge = require('../../../assets/images/imeng.png');

const clamp = (value: number, min: number, max: number) => Math.min(Math.max(value, min), max);

type ImEngQuestionRoute = RouteProp<RootStackParamList, 'ImEngQuestion'>;
...
const ImEngQuestionScreen: FC = () => {
  const route = useRoute<ImEngQuestionRoute>();
  ...
  // ImEng specific palette (can be customized)
  ...
  const handleSelectOption = useCallback(
    async (optionId: string) => {
      ...
      try {
        await imengService.recordAnswer(attemptId, currentQuestion.id, optionId);
      } catch (error) {
...
export default ImEngQuestionScreen;
